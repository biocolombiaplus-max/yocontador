-- Agrega el contenido real del landing original ("Bio Marketing" en GitHub):
-- las empresas Bio Contador y Bio Finanzas, y reemplaza el plan generico de
-- "Gestion de redes sociales" por los 4 planes reales con sus precios.
-- Ejecuta primero la migracion (columna isFeatured) y luego el resto.
-- Pega este script completo en el Editor SQL de Neon y ejecutalo una sola vez.

-- 0. Nueva columna requerida por el catalogo de servicios
...
-- 2. Tambien deja el link de demo de Biosoft (ya existia, solo le faltaba el website)
UPDATE "Company" SET "website" = 'https://bioauditoria.com/biosoft/landing.html' WHERE slug = 'biosoft' AND "website" IS NULL;

-- 3. Cuentas de redes sociales (pendientes de conectar) para las 2 empresas nuevas
...
-- 4. Oculta el plan generico anterior (no se borra por si algun cliente ya
--    esta suscrito a el; solo deja de mostrarse en la landing y en el panel)
UPDATE "ServiceOffering" SET "isActive" = false, "isPublished" = false
WHERE slug = 'gestion-redes-sociales';

-- 5. Los 4 planes reales de gestion de redes sociales
